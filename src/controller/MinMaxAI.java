...
public abstract class MinMaxAI extends Controller {
	...
	protected int score(Board b, int d, Player p) {
		if (d == depth)
			return estimate(b);
		if (b.getState() != State.NOT_OVER) return estimate(b);
		ArrayList<Integer> score = new ArrayList<Integer>();
		for (Location loc : moves(b)) {
			Board child = b.update(p, loc);
			score.add(score(child, d+1, p.opponent()));
		}
		int min = score.get(0);
		int max = score.get(0);
		for(Integer i: score) {
	...
	protected @Override Location nextMove(Game g) {
		...
		// choose a move use MinMax Algorithm
		if (!available.isEmpty()) {
			int score = -2147483648;
			Location l = null;
			for (Location loc : available) {
				Board child = g.getBoard().update(me, loc);
				int s = score(child, depth-1, me.opponent());
				if (s > score) {
					score = s;
					l = loc;
				}
